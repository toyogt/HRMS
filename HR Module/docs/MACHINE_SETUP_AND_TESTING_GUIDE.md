...
Assume project path: `<project-folder>`

In `configs/dev.yaml`, confirm:

1. `ingress_port: 9100` (or use another free port, but use same value everywhere).
...
If using port `9100`, replace `8080` with `9100` in all commands.
...
```powershell
cd "<project-folder>"
Test-Path ".\sdk_extracted\20211204-SBXPC-1\bin\SBXPCDLL64.dll"
curl.exe http://127.0.0.1:8080/health
```
